...
export default function EditPermintaanBarangPage() {
  ...
  const selectedBarang = availableBarang.find(
    (b) => b.id === formData.namaBarang
  )
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-4">
                  <Button variant="ghost" size="icon" asChild>
                    <Link href={`/transaksi/permintaan-barang/${params.id}`}>
                      <ArrowLeft className="h-4 w-4" />
                    </Link>
                  </Button>
                  <div className="flex flex-col gap-1">
                    <h1 className="text-2xl font-semibold tracking-tight">Edit Permintaan Barang</h1>
                    <p className="text-sm text-muted-foreground">
                      Ubah informasi permintaan barang yang ada. Pastikan semua data yang diubah sudah benar sebelum menyimpan.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedBarang && (
                        <p className="text-sm text-muted-foreground mt-1">
                          Barang terpilih: {selectedBarang.nama}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="flex justify-end gap-4 pt-4 border-t">
                    <Button type="button" variant="outline" asChild>
                      <Link href={`/transaksi/permintaan-barang/${params.id}`}>Batal</Link>
                    </Button>
        ...
        <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Apakah Anda yakin ingin menyimpan perubahan data permintaan barang ini? Pastikan semua informasi sudah benar sebelum menyimpan.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
